...
const DataStatusIndicator = () => {
  const { loading, error, lastUpdated, prices, inventory, machines } = useData();
  ...
  const getDataSummary = () => {
    if (!prices.length || !inventory || !machines) return 'No data';
    
    const totalMachines = (machines.air_miners || 0) + 
