...
const MarkdownRenderer = ({ content, className = '' }) => {
  const renderMarkdown = (text) => {
    ...
    formatted = formatted.replace(/^([•\-])\s(.+)$/gm, '<div class="flex items-start space-x-2 my-1"><span class="text-orange-400 text-sm mt-1">•</span><span>$2</span></div>');
    ...
    formatted = formatted.replace(/^([A-Z][^:]*):$/gm, '<div class="font-medium text-orange-300 mt-3 mb-2 text-sm">$1:</div>');
    
    // Handle numbered sections
    formatted = formatted.replace(/^\*\*([^*]+)\*\*$/gm, '<div class="font-medium text-orange-300 mt-3 mb-2">$1</div>');
    ...
    formatted = formatted.replace(/([\d.]+%)/g, '<span class="font-mono text-amber-400 bg-amber-500/10 px-1 rounded">$1</span>');
    ...
    formatted = formatted.replace(/\*\*([A-Z\s]+)\*\*:/g, '<span class="font-semibold text-orange-400">$1:</span>');
